...
static ofVec3f
interpolate_color(float a, const float *c1, const float *c2)
{
    ofVec3f c;
...
static ofVec3f colorTemp(int temp, float intensity){
    int temp_index = ((temp - 1000) / 100)*3;
    float alpha = (temp % 100) / 100.0;
    
	ofVec3f color = intensity*interpolate_color(alpha, &blackbody_color[temp_index],
