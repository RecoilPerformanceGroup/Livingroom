...
-(id)init{
    if(self = [super init]){
        ...
        [[self addPropF:@"iterations"] setMinValue:1 maxValue:100];        
    }
                            ...
                            do {
                                if((Halfedge_handle) curr != eit){
                                    ...
                                    if(curr->face()->is_unbounded() || curr->twin()->face()->is_unbounded()){
                                        //Edge is not OK
                                    } else if(v.size() > 0 && handleToVec2(u).distance(v[0]) > trackerRadius){
